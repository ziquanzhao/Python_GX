...
Traitdata <- read.xlsx('YourtraitData.xlsx',sheet = 'Sheet1', colNames = T, rowNames = T)
#读取表型数据
collectGarbage()
#内存垃圾回收，清理内存用的
...
dynamicMods <- cutreeDynamic(dendro = geneTree, distM = dissTOM,
                            ...
                            minClusterSize = 30)#一个模块中的基因如果数量太少的话，其实意义不大，因此我们规定了模块最小基因数量为30
...
#当然，也可以选择不绘图
module <- "darkturquoise" #选择你感兴趣的模块颜色
column <- match(module, modNames)
moduleGenes <- moduleColors==module
pdf(file = 'My_instering_model_darkturquoise.pdf', height = 10, width = 10)#注意PDF文件命名和感兴趣的模块颜色要一致
par(mfrow = c(1,1))
...
#输出模块结果
names(FpkmData)
names(FpkmData)[moduleColors=="darkturquoise"]#选择你感兴趣的模块颜色
...
FilterGenes <- abs(darkturquoise_KME$KME) > 0.7#kME阈值
darkturquoise_hub = subset(darkturquoise_KME, abs(darkturquoise_KME$KME)>0.7)#kME阈值
