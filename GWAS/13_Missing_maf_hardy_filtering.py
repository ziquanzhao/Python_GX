...
with open('13_Missing_rate_filtering.sh', 'a') as filtering:
    ...
    for filename in plink_bfilename:
        if '.bed' in filename:
            ...
            filtering.write(f'{plink2_software_path} --bfile {plink_bfile_path}{filename} --make-bed --allow-extra-chr --geno 0.1 --mind 0.02 --maf 0.05 --hwe 1e-4 --out {filename}_plink_filtering --threads 50\n')
...
#--geno 0.1 --mind 0.02 --maf 0.05 --hwe 1e-4,这根据13_Missing_rate_statistics.py绘图结果设置。GWAS项目采用的MAF阈值在0.01-0.05之间，取决于样本大小
