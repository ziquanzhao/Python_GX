...
with open('13_Missing_rate_filtering.sh', 'a') as filtering:
    ...
    for filename in plink_bfilename:
        if '.bed' in filename:
            ...
            filtering.write(f'{plink2_software_path} --bfile {plink_bfile_path}{filename} --make-bed --allow-extra-chr --geno 0.1 --mind 0.1 --maf 0.05 --hwe 0.01 --vcf-max-dp 50 --vcf-min-dp 3 --vcf-min-gq 10 --min-alleles 2 --max-alleles 2 --var-min-qual 30 --out {filename}_plink_filtering --threads 50\n')
...
#--recode，输出正常plink文件。
#--vcf-max-dp 50 --vcf-min-dp 3.位点最大深度50，最小深度3.一般来说，最下深度是平均深度的三分之一到四分之一。最大深度是平均深度的3到5倍。
