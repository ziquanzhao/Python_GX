...
with open('gvcflist.txt', 'r') as gvcf_list:
    for i in gvcf_list.readlines():
        for y in range(1, 16, 1):
            gcvf = f'_sort_dedup_chr{y}.g.vcf'
            if gcvf in i and 'idx' not in i:
                with open(f'chr{y}.txt', 'a') as CHR:
                    CHR.write(f'-V {all_gvcf}{i} ')

for chr in range(1, 16, 1):
    with open(f'chr{chr}.txt', 'r') as file:
        V = file.read().replace('\n', '')
        with open(f'7_gvcf-vcf_{chr}.sh', 'a') as file2:
            file2.write('#!/bin/bash\n')
            file2.write(f'java -jar {GATK_sorfware_path} CombineGVCFs -R {ref_path} {V} -O all_chr{chr}.g.vcf\n')
            file2.write(f'java -jar {GATK_sorfware_path} GenotypeGVCFs -R {ref_path} -V all_chr{chr}.g.vcf -O chr{chr}.vcf\n')
